...
function FactoryForm({ onAdded }) {
  ...
  const validateGstin = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5001/api/factories/validate-gstin', {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showNotification('success', 'Factory added successfully!');

      if (onAdded) onAdded();
    } catch (error) {
      console.error(error);
      showNotification('error', 'Failed to add factory. Please try again.');
    }
  };

  return (
    <div className="card">
      <h2 className="text-lg font-semibold text-gray-900 mb-6">Add New Factory</h2>
